      ...
      if (response?.status === 0) {
        setSelectedCandidate((prev: any) =>
          prev
            ? {
              ...prev,
              documentStatus: docMapStatus(statusId),
              documentVerifyStatusId: statusId,
            }
            : prev
        )
      }
        ...
        {selectedCandidate ? (
          <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="lg:col-span-2 space-y-6">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-medium border ${getStatusColor(selectedCandidate.biometricStatus)}`}>
                          ...
                          {selectedCandidate.biometricStatus == "Rejected" && <AlertCircle className="h-3 w-3" />}
              ...
              {selectedCandidate.biometricVerifyStatusId != 10 && (
                <div className="px-5">
                  <div className="mb-2 inline-flex items-center gap-2 px-3 py-2 rounded-md bg-amber-50 border border-amber-200 text-amber-700 text-sm">
                    <AlertCircle className="h-4 w-4" />
                    <span>Verify biometric first</span>
                  </div>
                </div>
              )}
              <div className={`bg-white rounded-lg border border-gray-200 overflow-hidden ${selectedCandidate.biometricVerifyStatusId === 10 ? '' : 'opacity-50 pointer-events-none'}`}>
                ...
                <div className="p-5">
                  <div className="flex items-center justify-between p-4 rounded-md border border-gray-200 bg-gray-50">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button onClick={async () => { await updateCandidateVerifyStatus(20); toast({ title: "Document Verified", description: "Candidate documents marked as verified." }) }} className="flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium text-white bg-orange-600 hover:bg-orange-700 transition-colors">
        ...
        <DialogContent className="bg-white">
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-end gap-2">
              ...
              <button disabled={submittingRemarks || !remarks.trim()} onClick={async () => { setSubmittingRemarks(true); await updateCandidateVerifyStatus(25, remarks.trim()); setSubmittingRemarks(false); setRemarksOpen(false); setRemarks(""); }} className="px-4 py-2 rounded-md bg-rose-600 text-white disabled:opacity-50">Submit</button>
            </div>
          </div>
        </DialogContent>
      </Dialog>
